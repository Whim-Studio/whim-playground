...
/**
 * PLACEHOLDER (owned by the Market/Venice UI task, T4). The city market screen:
 * a per-good table showing local buy/sell prices and stock, with buy/sell controls
 * that move goods between the selected docked unit and the city
 * (GAME_DESIGN_REFERENCE §3). Prices come from {@code PricingEngine}.
 */
public class MarketScreen extends Screen {
    ...
    public MarketScreen(Game game) {
        ...
        nav.add(back);
        nav.add(UiKit.label("Market — to be implemented (T4)", UiKit.HEAD, Palette.INK));
